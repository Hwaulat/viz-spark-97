...
function BagFilterHistory() {
  ...
  const [activeUF1Tab, setActiveUF1Tab] = useState("Pressure");
  const [activeUF2Tab, setActiveUF2Tab] = useState("Pressure");
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from({ length }, (_, i) => {
      ...
      return {
        ...
        uf2Cond: +(Math.random() * 20 + 80).toFixed(1),
        uf1: +(Math.random() * 5 + 25).toFixed(1),
        uf1Cond: +(Math.random() * 20 + 80).toFixed(1),
        uf1Module: +(Math.random() * 10 + 50).toFixed(1),
        ced1: +(Math.random() * 5 + 28).toFixed(1),
        ced2: +(Math.random() * 5 + 28).toFixed(1),
        cedFluidCond: +(Math.random() * 50 + 200).toFixed(1),
  ...
  const renderChart = (dataKey: string, name: string, color: string, yAxisLabel: string = "Pressure (MPa)") => {
    const isCond = dataKey.endsWith("Cond");
    return (
        ...
        <div className="flex-1 min-h-0 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 10, right: 10, left: 10, bottom: 0 }}>
              ...
              {!isCond && <Line type="monotone" dataKey="maxLimit" name="Max" stroke="#ef4444" strokeWidth={1} strokeDasharray="5 5" dot={false} activeDot={false} />}
              {!isCond && <Line type="monotone" dataKey="minLimit" name="Min" stroke="#ef4444" strokeWidth={1} strokeDasharray="5 5" dot={false} activeDot={false} />}
              {isCond && <Line type="monotone" dataKey="stdLimit" name="Standard" stroke="#ef4444" strokeWidth={1} strokeDasharray="5 5" dot={false} activeDot={false} />}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6 mb-6">
        {activeTab === "CED Line" ? (
          <>
            ...
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm flex flex-col h-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 shrink-0">
                <h2 className="text-xl font-bold tracking-tight text-foreground">Trend UF 2</h2>
                <div className="flex bg-gray-100 dark:bg-gray-700/60 rounded-xl p-1 shrink-0">
                  {["Pressure", "Conductivity"].map(tab => (
                    <button
                      key={tab}
                      onClick={() => setActiveUF2Tab(tab)}
                      className={`text-gray-500 dark:text-gray-400 rounded-lg text-xs font-medium h-8 px-4 whitespace-nowrap transition-colors ${activeUF2Tab === tab
                        ? "bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm"
                        : "hover:text-gray-700 dark:hover:text-gray-300"
                        }`}
                    >{tab}</button>
              ...
              <div className="h-[250px] w-full">
                {activeUF2Tab === "Pressure" 
                  ? renderChart("uf2", "Pressure UF 2 (MPa)", "#10b981", "Pressure (MPa)")
                  : renderChart("uf2Cond", "Conductivity UF 2 (µS)", "#10b981", "Conductivity (µS)")}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm flex flex-col h-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 shrink-0">
                <h2 className="text-xl font-bold tracking-tight text-foreground">Trend UF 1</h2>
                <div className="flex bg-gray-100 dark:bg-gray-700/60 rounded-xl p-1 shrink-0">
                  {["Pressure", "Conductivity", "UF Module"].map(tab => (
                    <button
                      key={tab}
                      onClick={() => setActiveUF1Tab(tab)}
                      className={`text-gray-500 dark:text-gray-400 rounded-lg text-xs font-medium h-8 px-4 whitespace-nowrap transition-colors ${activeUF1Tab === tab
                        ? "bg-white dark:bg-gray-800 text-gray-900 dark:text-white shadow-sm"
                        : "hover:text-gray-700 dark:hover:text-gray-300"
                        }`}
                    >{tab}</button>
              ...
              <div className="h-[250px] w-full">
                {activeUF1Tab === "Pressure" && renderChart("uf1", "Pressure UF 1 (MPa)", "#f59e0b", "Pressure (MPa)")}
                {activeUF1Tab === "Conductivity" && renderChart("uf1Cond", "Conductivity UF 1 (µS)", "#f59e0b", "Conductivity (µS)")}
                {activeUF1Tab === "UF Module" && renderChart("uf1Module", "UF Module (L/Min)", "#f59e0b", "UF Module (L/Min)")}
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm flex flex-col h-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 shrink-0">
                ...
                <div className="flex bg-gray-100 dark:bg-gray-700/60 rounded-xl p-1 shrink-0">
                  {["CED 1", "CED 2", "Fluid Level"].map(tab => (
                    <button
                      key={tab}
              ...
              <div className="h-[250px] w-full">
                {activeCEDTab === "Fluid Level"
                  ? renderChart("cedFluidCond", "Fluid Level (mm)", "#8b5cf6", "Fluid Level (mm)")
          ...
          <>
            ...
            <div className="rounded-xl border border-border bg-card p-6 shadow-sm flex flex-col h-full">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 shrink-0">
                <h2 className="text-xl font-bold tracking-tight text-foreground">Pressure Trend HE Phosphate</h2>
                <div className="flex bg-gray-100 dark:bg-gray-700/60 rounded-xl p-1 shrink-0">
                  {["IN", "OUT"].map(tab => (
                    <button
                      key={tab}
