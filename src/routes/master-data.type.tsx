...
export const Route = createFileRoute("/master-data/type")({
  head: () => ({
    meta: [
      { title: "Master Data — Type" },
      { name: "description", content: "Master Data for Type." },
    ],
  }),
  component: TypePage,
});
...
const INITIAL_DATA: TypeData[] = [
  { id: "T-001", name: "TD" },
  { id: "T-002", name: "TD-WIDE" },
  { id: "T-003", name: "TA" },
];

function TypePage() {
  const [data, setData] = useState<TypeData[]>(INITIAL_DATA);
  ...
  const openEditModal = (item: TypeData) => {
    setEditId(item.id);
  ...
  const saveModal = () => {
    ...
    if (editId) {
      ...
      setData(prev => [...prev, { id: "T-" + Date.now(), name: formName }]);
    }
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-secondary/40 text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/50">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold">type Name</th>
              </tr>
            </thead>
