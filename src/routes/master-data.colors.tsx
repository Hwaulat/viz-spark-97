...
export const Route = createFileRoute("/master-data/colors")({
  head: () => ({
    meta: [
      { title: "Master Data — Colors" },
      { name: "description", content: "Master Data for Colors." },
    ],
  }),
...
const INITIAL_DATA: ColorData[] = [
  { id: "C-001", code: "CL-001", name: "Colors A" },
  { id: "C-002", code: "CL-002", name: "Colors A" },
  { id: "C-003", code: "CL-003", name: "Colors A" },
  { id: "C-004", code: "CL-004", name: "Colors A" },
  { id: "C-005", code: "CL-005", name: "Colors A" },
];

function ColorsPage() {
  const [data, setData] = useState<ColorData[]>(INITIAL_DATA);
  ...
  const openEditModal = (item: ColorData) => {
    setEditId(item.id);
  ...
  const saveModal = () => {
    ...
    if (editId) {
      setData(prev => prev.map(item => item.id === editId ? { ...item, code: formCode, name: formName } : item));
    } else {
      setData(prev => [...prev, { id: "C-" + Date.now(), code: formCode, name: formName }]);
    }
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-secondary/40 text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/50">
              <tr>
                ...
                <th className="px-6 py-3 font-semibold w-48">Colors Code</th>
                <th className="px-6 py-3 font-semibold">Colors Name</th>
              </tr>
            </thead>
        ...
        <DialogContent className="sm:max-w-[500px] p-0 overflow-hidden">
          ...
          <div className="p-6 space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Color Code</label>
              <input 
                value={formCode}
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Color Name</label>
              <input 
                value={formName}
                onChange={(e) => setFormName(e.target.value)}
                placeholder="e.g., Colors A"
                className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm outline-none focus:border-primary transition-colors"
              />
            </div>
          </div>
